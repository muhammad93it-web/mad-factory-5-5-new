...
import { DollarSign, Settings, Bell, Factory, Home, ChevronRight, LogOut, Users, ShieldCheck, User, HardDriveDownload, Maximize, Minimize, Download, X } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col" dir="rtl">
      ...
      {!isHome && (
        <div className="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 px-6 py-2.5 print:hidden">
          <div className="max-w-7xl mx-auto text-xs text-slate-500 flex items-center gap-1.5">
            <Link href="/" className="hover:text-primary transition-colors">سەرەکی</Link>
            <ChevronRight className="h-3 w-3 rotate-180" />
            <span className="text-slate-700 dark:text-slate-300 font-medium">{titleFromPath(location)}</span>
          </div>
        </div>
      )}
