...
import {
  ArrowRight, Printer, FileSpreadsheet, Truck, Phone, MapPin, FileText, Calendar,
} from "lucide-react";
...
export default function SupplierDetail() {
  ...
  const { data: supplier, isLoading: loadingS } = useGetSupplier(id, {
    query: { enabled: !!id, queryKey: getGetSupplierQueryKey(id) },
  });
  const { data: statement, isLoading: loadingStmt } = useGetSupplierStatement(
    id,
    { fromDate: applied.fromDate, toDate: applied.toDate },
    { query: { enabled: !!id, queryKey: getGetSupplierStatementQueryKey(id, { fromDate: applied.fromDate, toDate: applied.toDate }) } },
  );

  if (loadingS) return <div className="p-8 text-center text-slate-500">بەڕێکردن...</div>;
  if (!supplier) return <div className="p-8 text-center text-destructive">دابینکار نەدۆزرایەوە</div>;
  ...
  const handleExport = () => {
    ...
    const html = `
      <h2 style="text-align:right">کەشف حسابی دابینکار: ${supplier.name}</h2>
    ...
    exportTableToExcel(`statement-supplier-${supplier.id}.xls`, html);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between print:hidden">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
          <Truck className="h-6 w-6 text-primary" />کەشف حسابی دابینکار
        </h1>
        ...
        <CardContent className="p-8 space-y-6">
          <div className="flex items-start justify-between pb-5 border-b-2 border-primary">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">کەشف حسابی دابینکار / Supplier Statement</p>
            </div>
            <div className="text-left">
              <div className="text-xs text-slate-500 mb-1">ناوی دابینکار</div>
              <div className="text-xl font-bold text-primary">{supplier.name}</div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <div className="font-medium" dir="ltr">{supplier.phone || "-"}</div>
            </div>
            <div>
              <div className="text-xs text-slate-500 flex items-center gap-1.5 mb-1"><Phone className="h-3 w-3" />مۆبایل ٢</div>
              <div className="font-medium" dir="ltr">{supplier.phone2 || "-"}</div>
            </div>
                  ...
                  statement!.entries.map((e, i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-white dark:bg-slate-950" : "bg-slate-50 dark:bg-slate-900"}>
                      ...
                      <td className="border border-slate-300 dark:border-slate-700 px-3 py-2">
                        {(e.type === "invoice" || e.type === "purchase") && e.referenceId ? (
                          <Link href={`/purchases/${e.referenceId}`} className="text-primary hover:underline">{e.description}</Link>
                        ) : e.description}
                      </td>
